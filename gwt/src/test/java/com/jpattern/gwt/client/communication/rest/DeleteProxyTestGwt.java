 ...
 * 12 Apr 2011
 */
public class DeleteProxyTestGwt extends BaseGwtTestCase {
	...
	public void testDeleteProxy() throws Exception {
		  Timer timer = new Timer() {
				...
				public void run() {
					...
					AProxy<IWebResultResultObject> proxy = new DeleteProxy<IWebResultResultObject>(new MyCallback(), new AutoBeanObjectSerializer<IWebResultResultObject>(getBeanFactory(), IWebResultResultObject.class, getProvider()) , url, keyValuesMap, getProvider());
					try {
						proxy.call();
					} catch (Exception e) {
						e.printStackTrace();
