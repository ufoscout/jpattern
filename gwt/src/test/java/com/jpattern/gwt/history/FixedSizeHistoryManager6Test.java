 ...
 * 08/ago/2011
 */
public class FixedSizeHistoryManager6Test extends BaseTest {
	...
	private final List<String> calledEventList = new ArrayList<String>();
	
	@Override
	protected void setUp() throws Exception {
		SimplePresenter.ID_COUNT = 0;
	...
	public void testHistoryManager1() throws Exception {
		...
		MockMainNavigationEvent mainNavigationEvent = new MockMainNavigationEvent("root");
		HasWidgets targetArea = new NullHasWidgets();
		mainNavigationEvent.registerNavigationEvent(new NavEvent("event1"), targetArea, new NullNavigationEventCallback());
		mainNavigationEvent.registerNavigationEvent(new NavEvent("event2"), targetArea, new NullNavigationEventCallback());
		mainNavigationEvent.registerNavigationEvent(new NavEvent("event3"), targetArea, new NullNavigationEventCallback());
		mainNavigationEvent.registerNavigationEvent(new NavEvent("event4"), targetArea, new NullNavigationEventCallback());
		mainNavigationEvent.addInitialNavigationEvent("event1");
		...
		assertEquals( "root_event2" , historyManagerObserver.eventName );
		assertEquals( 2 , createdPresenterList.size() );
		assertEquals( "event1", createdPresenterList.get(0).getNavigationEvent().getName());
		assertEquals( "event2", createdPresenterList.get(1).getNavigationEvent().getName());
		assertEquals( 1, ((SimplePresenter)createdPresenterList.get(1)).id);
		
		assertEquals( 1, calledEventList.size() );
		assertEquals( "event2", calledEventList.get(0) );
		calledEventList.clear();
		createdPresenterList.clear();
		
		IHistoryManager historyManager = applicationProvider.getHistoryService().getHistoryManager();
		
		historyManager.onEvent( "root_event3_" );
		assertEquals( "root_event3_" , historyManagerObserver.eventName );
		assertEquals( 1 , createdPresenterList.size() );
//		assertEquals( "root", createdPresenterList.get(0).getNavigationEvent().getName());
		assertEquals( "event3", createdPresenterList.get(0).getNavigationEvent().getName());
		
		assertEquals( 1, calledEventList.size() );
		assertEquals( "event3", calledEventList.get(0) );
		calledEventList.clear();
