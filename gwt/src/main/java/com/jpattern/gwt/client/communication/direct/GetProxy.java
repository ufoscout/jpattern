...
public class GetProxy<T extends ICommandFacadeResult<?>> extends AProxy<T> {
	...
	public GetProxy(IServerCallGetAction serverCallAction, ICallbackAction<T> callbackAction, Class<T> resultClass, String url, Map<String, String> keyValuesMap, IApplicationProvider provider) {
	...
	protected void execute(ICallbackAction<T> callbackAction, IApplicationProvider provider) throws Exception {
		...
		try {
			callbackAction.onSuccess( serverCallAction.get(resultClass, url, keyValuesMap) , 0 );
		} catch (Exception e) {
