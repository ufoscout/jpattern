 ...
 * 06/mag/2011
 */
public class ServerCallPostCommand<T extends ICommandFacadeResult<?>, Z> extends AServerCallCommand<T> {

	private final StringBuffer url;
	private final GenericWrapper<T> callResult;
	private final GenericWrapper<Z> dataInput;
	
	public ServerCallPostCommand(GenericWrapper<Z> dataInput, StringBuffer url, GenericWrapper<T> callResult) {
		this.url = url;
	...
	protected void exec(ICommandResult commandResult) {
		...
		AProxy<T> proxy = getProvider().getServerCallService().post(resultSerializer, dataSerializer, commandCallBack, url.toString(), dataInput.getValue());
		try {
			proxy.call();
			...
			logger.error("exec", "Exception on Post Server call", exception);
		}
	}
