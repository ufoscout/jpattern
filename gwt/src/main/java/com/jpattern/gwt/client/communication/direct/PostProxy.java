...
public class PostProxy<T extends ICommandFacadeResult<?>,Z> extends AProxy<T> {
	...
	public PostProxy(IServerCallPostAction serverCallPostAction, Class<T> resultClass, Class<Z> dataClass, ICallbackAction<T> callbackAction, String url, Z data, IApplicationProvider provider) {
	...
	protected void execute(ICallbackAction<T> callbackAction, IApplicationProvider provider) throws Exception {
		...
		try {
			callbackAction.onSuccess( serverCallPostAction.post(resultClass, dataClass, url, data) ,0 );
		} catch (Exception e) {
